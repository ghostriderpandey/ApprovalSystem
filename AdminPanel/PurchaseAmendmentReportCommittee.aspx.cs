...
public partial class AdminPanel_PurchaseAmendmentReportCommittee : System.Web.UI.Page
{
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PANCommitteeApprove','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PANCommitteeReject','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
    ...
    public void DownloadDocImage(string ID, string PANID, string DocFile, string Type)
    {
        ...
        if (DocFile != "" && File.Exists(Server.MapPath("../Upload/PAN/") + DocFile))
        {
            ...
            dtPANOrderNo = cls.selectDataTable(" select b.PANOrderNo from tbl_PANCommitteeApproval a inner join tbl_PurchaseAmendmentNote b on a.PANID=b.ID Where a.IsApprove=1 And  a.PANID='" + PANID + "' and a.IsDelete=0");
    ...
    private void ManipulatePdf(string src)
    {
        PdfReader reader = new PdfReader(Server.MapPath("../Upload/PAN/") + src);
        PdfStamper stamper = new PdfStamper(reader, new FileStream(Server.MapPath("../Upload/PAN/10001.pdf"), FileMode.Create));
        int n = reader.NumberOfPages;
        iTextSharp.text.Rectangle pagesize;
        for (int i = 1; i <= n; i++)
        {
            ...
            ColumnText.ShowTextAligned(over, Element.ALIGN_CENTER, new Phrase("Watermark", new iTextSharp.text.Font(iTextSharp.text.Font.HELVETICA, 50)), x, y, 0);
            over.RestoreState();
        }
        stamper.Close();
        reader.Close();
