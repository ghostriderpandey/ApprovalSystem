...
public partial class AdminPanel_ListPurchaseAmendmentNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManagePurchaseAmendmentNote 'GetAll','" + dtUser.Rows[0]["ID"].ToString() + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec sp_ManagePurchaseAmendmentNote 'ChangeStatus','" + e.CommandArgument + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Amendment Successfully Update !!');", true);
                loadgvUser();
            }
        }
        else if (e.CommandName == "IsDelete")
        {
            int count = cls.ExecuteQuery("Exec sp_ManagePurchaseAmendmentNote 'Delete','" + e.CommandArgument + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Amendment Successfully Deleted !!');", true);
                loadgvUser();
            }
        }
