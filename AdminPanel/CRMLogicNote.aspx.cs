...
public partial class AdminPanel_CRMLoginNote : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            HttpContext.Current.Session["CRMImage"] = null;
            HttpContext.Current.Session["CRMFile"] = null;
    ...
    {
        dtData = cls.selectDataTable("EXEC sp_ManageCRMLogicNote 'GetByID','" + ID + "'");
        if (dtData.Rows.Count > 0)
        {
            ...
            //ddlprojectname_SelectedIndexChanged(null, null);
            //txtProjectaddress.Text = dtData.Rows[0]["Address"].ToString();
            ...
            if (dtData.Rows[0]["CRMOrderNo"].ToString() != "")
            {
                txtApprovalNo.Text = dtData.Rows[0]["CRMOrderNo"].ToString();
            }
            else
            {
            ...
            int status = cls.ExecuteIntScalar("select count(1) from tbl_CRMLogicNote where ID=" + ID + " and IsSubmitted=1");
            if (status == 0)
            {
            ...
            {
                status = cls.ExecuteIntScalar("select count(1) from tbl_CRMLogicNote where ID=" + ID + " and IsCommitteeApproved=1");
                if (status == 0)
                {
    ...
    {
        string orderNo = cls.ExecuteStringScalar("select ApprovalNo from tbl_CRMLogicNote where CRMOrderNo='" + txtApprovalNo.Text + "' and CRMOrderNo like '%R0%'");
        return orderNo;
    }
    ...
    {
        DataTable dtData = cls.selectDataTable("select ID,PaymentTerms,Amount,[Description],Remark from tbl_CRMPaymentPlan where IsDelete=0 and CRMID='" + ID + "'");
        if (dtData.Rows.Count > 0)
        {
    ...
    {
        DataTable dtData = cls.selectDataTable("select ID,Description,Amount,PaymentMode,Remark from tbl_CRMPaymentDetails where IsDelete=0 and CRMID='" + ID + "'");
        if (dtData.Rows.Count > 0)
        {
            ViewState["dtPaymentDetails"] = dtData;
    ...
    protected void gvApprover_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Delete")
        {
            ...
            {
                cls.ExecuteQuery("EXEC sp_ManageCRMLogicNote 'DeleteCRMApprover','" + e.CommandArgument + "'");
                DataTable dtData = (DataTable)ViewState["dtDetailsApprover"];
                dtData.Rows.RemoveAt(RowIndex);
                ViewState["dtDetailsApprover"] = dtData;
                gvApprover.DataSource = dtData;
    ...
    {
        DataTable dtData = cls.selectDataTable("select ID,ApproverID from tbl_CRMApprover where IsDelete=0 and CRMID='" + ID + "'");
        if (dtData.Rows.Count > 0)
        {
            ViewState["dtDetailsApprover"] = dtData;
        }
    ...
    {
        // ClientScript.RegisterStartupScript(this.GetType(), "SuccessMessage", "SetAttachment();", true);
        //System.Threading.Thread.Sleep(TimeSpan.FromSeconds(10));
        DataTable dtCRMAttachment = new DataTable();
        if (hdnAttachmentData.Value != "")
        {
            dtCRMAttachment = (DataTable)JsonConvert.DeserializeObject(hdnAttachmentData.Value, (typeof(DataTable)));
            ViewState["dtDetailsAttachment"] = dtCRMAttachment;
        }
        else
        {
            dtCRMAttachment.Columns.Add("ID");
            dtCRMAttachment.Columns.Add("Description");
            dtCRMAttachment.Columns.Add("DocFile");
            dtCRMAttachment.Columns.Add("DocImage");

            string strFileName = "";
            int i = 1;
            foreach (GridViewRow row in gvAttachment.Rows)
            {
                DataRow dr = dtCRMAttachment.NewRow();
                ...
                dtCRMAttachment.Rows.Add(dr);
                i = i + 1;

            }
            ViewState["dtDetailsAttachment"] = dtCRMAttachment;
        }
        gvAttachment.DataSource = dtCRMAttachment;
        gvAttachment.DataBind();

    }
    private void FillDtAttachment(string ID)
    {
        DataTable dtData = cls.selectDataTable("select ID,Description,DocFile,DocImage from tbl_CRMAttachment where IsDelete=0 and CRMID='" + ID + "'");
        if (dtData.Rows.Count > 0)
        {
            ViewState["dtDetailsAttachment"] = dtData;
        }
    ...
    {
        if (DocFile != "" && File.Exists(Server.MapPath("../Upload/CRM/") + DocFile))
        {
            if (Request.QueryString["ID"] != null)
            {
                string PLNID = Request.QueryString["ID"].ToString();
                DataTable dtPLNOrderNo = new DataTable();
                dtPLNOrderNo = cls.selectDataTable(" select b.CRMOrderNo from tbl_CRMApprover a inner join tbl_CRMLogicNote b on a.CRMID=b.ID Where a.IsApprove=1 And  a.CRMID='" + PLNID + "' and a.IsDelete=0");
                if (dtPLNOrderNo.Rows.Count > 0)
                {
                    if (dtPLNOrderNo.Rows.Count > 0)
                    {
                        string filePath = Server.MapPath("../Upload/CRM/") + ID + PLNID + Type + DocFile;
                        if (Type == "PDF")
                        {
                            PdfReader reader = new PdfReader(Server.MapPath("../Upload/CRM/") + DocFile);
                            ...
                            for (int i = 1; i <= n; i++)
                            {
                                ...
                                ColumnText.ShowTextAligned(over, Element.ALIGN_CENTER, new Phrase(dtPLNOrderNo.Rows[0]["CRMOrderNo"].ToString(), new iTextSharp.text.Font(iTextSharp.text.Font.TIMES_ROMAN, 15, iTextSharp.text.Font.BOLD)), pagesize.Width - 250, pagesize.Height - 30, 0);
                                over.RestoreState();
                            }
                            stamper.Close();
                            reader.Close();
                        ...
                        else if (Type == "Image")
                        {
                            ...
                            string imageFilePath = Server.MapPath("../Upload/CRM/") + DocFile;
                            Bitmap newBitmap;
                            using (var bitmap = (Bitmap)System.Drawing.Image.FromFile(imageFilePath))//load the image file
                            {
                                using (Graphics graphics = Graphics.FromImage(bitmap))
                                {
                                    using (System.Drawing.Font arialFont = new System.Drawing.Font("TIMES_ROMAN", 12, FontStyle.Bold))
                                    {
                                        ...
                                        graphics.DrawString(dtPLNOrderNo.Rows[0]["CRMOrderNo"].ToString(), arialFont, Brushes.DarkBlue, rect1);
                    ...
                    {
                        var filePath = Server.MapPath("../Upload/CRM/") + DocFile + "";
                        Response.ContentType = "application/octet-stream";
                        Response.AppendHeader("Content-Disposition", "attachment; filename=" + DocFile + "");
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["ID"] == null)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ManageCRMLogicNote"))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Connection = con;
                        cmd.Parameters.AddWithValue("@Action", "Insert");
                        cmd.Parameters.AddWithValue("@ID", "0");
                        cmd.Parameters.AddWithValue("@CRMRefID", "");
                        ...
                        cmd.Parameters.AddWithValue("@CRMApprover", dtSLNApprover);
                        cmd.Parameters.AddWithValue("@CRMAttachment", dtSLNAttachment);
                        cmd.Parameters.AddWithValue("@CRMPaymentPlan", dtSLNPaymentPlan);
                        cmd.Parameters.AddWithValue("@CRMPaymentDetails", dtSLNPaymnetDetails);
                        con.Open();
    ...
    private string UploadImage(string strID, FileUpload Fud)
    {
        ...
        if (Fud.HasFile)
        {
            ...
            url = strname;
            Fud.SaveAs(Server.MapPath("~/Upload/CRM/") + strname);
        }
        return url;
    }
    ...
    #region ProjectAddress And Location
    [WebMethod]
    ...
    {
        cls_connection_new cls = new cls_connection_new();
        DataTable dt = cls.selectDataTable("select CRMOrderNo from tbl_CRMLogicNote where CRMOrderNo like '%" + prefixText + "%' and IsActive=1 and CRMOrderNo is not null and CRMOrderNo<>''");
        List<string> list = new List<string>();
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            list.Add(dt.Rows[i]["CRMOrderNo"].ToString());
        }
        return list;
    }
    ...
    protected void txtApprovalNo_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            int ID = cls.ExecuteIntScalar("select ID From tbl_CRMLogicNote where CRMOrderNo='" + txtApprovalNo.Text + "'");
