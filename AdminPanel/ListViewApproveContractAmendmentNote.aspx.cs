...
public partial class AdminPanel_ListViewApproveContractAmendmentNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManageContractAmendmentNote 'GetViewAllforApproval',@ID='" + dtUser.Rows[0]["ID"].ToString() + "',@ProjectID='" + ddlprojectname.SelectedValue + "',@ApprovalStatus='" + ddlStatus.SelectedValue + "',@ApprovalTodate='" + txtToApprovalDate.Text + "',@ApprovalFromdate='" + txtFromApprovalDate.Text + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CANApproverApprove','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Contract Amendment Logic Note Successfully Approved !!');", true);
                loadgvUser();
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CANApproverReject','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Contract Amendment Logic Note Successfully Rejected !!');", true);
                loadgvUser();
            }
        }
        ...
        else if (e.CommandName == "View")
        {
            try
            {
                ...
                //_reportViewer.ServerReport.ReportPath = "/swapproval/Contract Amendment Logic Note";
