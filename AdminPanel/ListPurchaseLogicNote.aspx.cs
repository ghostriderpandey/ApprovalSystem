...
public partial class AdminPanel_ListPurchaseLogicNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManagePurchaseLogicNote 'GetAll','" + dtUser.Rows[0]["ID"].ToString() + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec sp_ManagePurchaseLogicNote 'ChangeStatus','" + e.CommandArgument + "'");
            if (count > 0)
            {
        ...
        {
            int count = cls.ExecuteQuery("Exec sp_ManagePurchaseLogicNote 'Delete','" + e.CommandArgument + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Logic Note Successfully Deleted !!');", true);
                loadgvUser();
            }
        }
