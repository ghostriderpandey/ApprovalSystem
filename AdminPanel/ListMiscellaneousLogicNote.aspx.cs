...
public partial class AdminPanel_ListMiscellaneousLogicNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManageMiscellaneousLogicNote 'GetAll','" + dtUser.Rows[0]["ID"].ToString() + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec sp_ManageMiscellaneousLogicNote 'ChangeStatus','" + e.CommandArgument + "'");
            if (count > 0)
            {
        ...
        {
            int count = cls.ExecuteQuery("Exec sp_ManageMiscellaneousLogicNote 'Delete','" + e.CommandArgument + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Miscellaneous Logic Note Successfully Deleted !!');fnn();", true);
                loadgvUser();
            }
        }
        else if (e.CommandName == "ViewApprover")
        {
            string title = "";
            dtData = new DataTable();
            dtData = cls.selectDataTable("EXEC sp_ManageMiscellaneousLogicNote 'GetAllApprover','" + e.CommandArgument + "'");
    ...
    [WebMethod]
    public static string IsStatus(string id)
    {
        string s = "failed";
        return s;
    }
