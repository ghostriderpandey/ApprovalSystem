...
public partial class AdminPanel_ListApproveCommitteePurchaseAmendmentNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManagePurchaseAmendmentNote 'GetAllforCommitteeApproval',@ID='" + dtUser.Rows[0]["ID"].ToString() + "',@ProjectID='" + ddlprojectname.SelectedValue + "',@ApprovalStatus='" + ddlStatus.SelectedValue + "',@ApprovalTodate='" + txtToApprovalDate.Text + "',@ApprovalFromdate='" + txtFromApprovalDate.Text + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PANCommitteeApprove','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Amendment Note Successfully Approved !!');", true);
                loadgvUser();
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PANCommitteeReject','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Amendment Note Successfully Rejected !!');", true);
                loadgvUser();
            }
        }
