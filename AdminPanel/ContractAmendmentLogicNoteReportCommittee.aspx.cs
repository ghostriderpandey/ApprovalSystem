...
public partial class AdminPanel_ContractAmendmentLogicNoteReportCommittee1 : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["ID"] != null)
            {
                ...
                if (dtData.Rows.Count > 0)
                {
                    ...
                    {
                        lblPurchaseOrderNo.Visible = false;
                    }
                    else
                    {
                        lblPurchaseOrderNo.Visible = true;
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CANCommitteeApprove','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CANCommitteeReject','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
    ...
    public void DownloadDocImage(string ID, string CANID, string DocFile, string Type)
    {
        ...
        if (DocFile != "" && File.Exists(Server.MapPath("../Upload/CAN/") + DocFile))
        {
            ...
            dtCANOrderNo = cls.selectDataTable(" select b.CANOrderNo from tbl_CANCommitteeApproval a inner join tbl_ContractAmendmentNote b on a.CANID=b.ID Where a.IsApprove=1 And  a.CANID='" + CANID + "' and a.IsDelete=0");
                ...
                try
                {
                    if (Type == "PDF")
                    {
                        ...
                        for (int i = 1; i <= n; i++)
                        {
                            ...
                            ColumnText.ShowTextAligned(over, Element.ALIGN_CENTER, new Phrase(dtCANOrderNo.Rows[0]["CANOrderNo"].ToString(), new iTextSharp.text.Font(iTextSharp.text.Font.HELVETICA, 25)), pagesize.Width - 250, pagesize.Height - 30, 0);
                            over.RestoreState();
                        }
                        stamper.Close();
                        reader.Close();
