...
public partial class AdminPanel_ListApproveCommitteeContractAmendmentNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManageContractAmendmentNote 'GetAllforCommitteeApproval',@ID='" + dtUser.Rows[0]["ID"].ToString() + "',@ProjectID='" + ddlprojectname.SelectedValue + "',@ApprovalStatus='" + ddlStatus.SelectedValue + "',@ApprovalTodate='" + txtToApprovalDate.Text + "',@ApprovalFromdate='" + txtFromApprovalDate.Text + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CANCommitteeApprove','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Contract Amendment Note Successfully Approved !!');", true);
                loadgvUser();
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CANCommitteeReject','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Contract Amendment Note Successfully Rejected !!');", true);
                loadgvUser();
            }
        }
