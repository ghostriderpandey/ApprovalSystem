...
public partial class AdminPanel_ListViewApprovePurchaseLogicNote_aspx : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManagePurchaseLogicNote 'GetViewAllforApproval','" + dtUser.Rows[0]["ID"].ToString() + "',@ProjectID='" + ddlprojectname.SelectedValue + "',@ApprovalStatus='" + ddlStatus.SelectedValue + "',@ApprovalTodate='" + txtToApprovalDate.Text + "',@ApprovalFromdate='" + txtFromApprovalDate.Text + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PLNApproverApprove','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Logic Note Successfully Approved !!');", true);
                loadgvUser();
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PLNApproverReject','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Purchase Logic Note Successfully Rejected !!');", true);
                loadgvUser();
            }
        }
        else if (e.CommandName == "ViewApprover")
        {
            string title = "";
            dtData = new DataTable();
            dtData = cls.selectDataTable("EXEC sp_ManagePurchaseLogicNote 'GetAllApprover','" + e.CommandArgument + "'");
        ...
        else if (e.CommandName == "View")
        {
            try
            {
                ...
                Export.ExportReport(e.CommandArgument.ToString(), "Purchase Logic Note");
                //string ReportType = "P";
                //Warning[] warnings;
                //string[] streamIds;
