<%@ Page Title="" Language="C#" MasterPageFile="~/AdminPanel/AdminPanel.master" AutoEventWireup="true" CodeFile="ListApproveCommitteeSalesLogicNote.aspx.cs" Inherits="AdminPanel_ListApproveCommitteeSalesLogicNote" %>
        ...
        <ContentTemplate>--%>
                ...
                <div class="w-100">
                  <asp:GridView ID="gvUser" ShowHeaderWhenEmpty="true" EmptyDataText="No Record Found" runat="server" AutoGenerateColumns="False" CssClass="table  table-bordered table-responsive" OnRowCommand="gvUser_RowCommand">
                    ...
                    <asp:BoundField DataField="SLNOrderNo" HeaderText="SerialCode" />
                    ...
                    <asp:TemplateField HeaderText="Action">
                      <ItemTemplate> <a href="SalesLogicNoteReport.aspx?type=committe&id=<%#Eval("SLNID") %>&AID=<%#Eval("ID") %>" title="Click to view" class="fa fa-eye  "></a>
                        <asp:LinkButton ID="lnkView" ToolTip="Click to view..!" runat="server" CommandName="View" CommandArgument='<%#Eval("SLNID") %>'><img src="Icons/view_16x16.png" /></asp:LinkButton>
                        <%-- <a href="PurchaseLogicNote.aspx?id=<%#Eval("ID") %>" title="Edit this record">
                                                        <img src="icons/edit_16x16.png" alt="Edit" />
                                                    </a>--%>
                        <asp:LinkButton ID="lnkApprove" ToolTip="Click to approve..!" runat="server" Style='<%#Eval("ApprovalStatus").ToString()=="Pending"?"Display:contents": "Display:none" %>' OnClientClick="return ApproveItem();" Text="Approve" CommandName="Approve" CommandArgument='<%#Eval("ID") %>'> <img src="Icons/Approve.png" /> </asp:LinkButton>
                        <asp:LinkButton ID="lnkReject" ToolTip="Click to reject..!" runat="server" OnClientClick="return RejectItem();" Style='<%#Eval("ApprovalStatus").ToString()=="Pending"?"Display:contents": "Display:none" %>' Text="Reject" CommandName="Reject" CommandArgument='<%#Eval("ID") %>'> <img src="Icons/Reject.png" /> </asp:LinkButton>
                      </ItemTemplate>
                    </asp:TemplateField>
                    </Columns>
                  </asp:GridView>
