...
public partial class AdminPanel_CRMLogicNoteReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Request.QueryString["ID"] != null)
            {
                ...
                dtData = cls.selectDataTable("EXEC sp_ManageCRMLogicNote 'View','" + Request.QueryString["ID"].ToString() + "'");
                if (dtData.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        lblorderno.Text = dtData.Rows[0]["CRMOrderNo"].ToString();
                    }
                    ...
                    if (dtData.Rows[0]["ApprovalType"].ToString() == "2")
                    {
                        ...
                        lblApprovalNo.Text = dtData.Rows[0]["CRMOrderNo"].ToString();
                        spaNo.Visible = true;
                    ...
                    int status = cls.ExecuteIntScalar("select count(1) from tbl_CRMLogicNote where ID=" + Convert.ToInt32(Request.QueryString["ID"]) + " and IsSubmitted=1");
                    if (status == 0)
                    {
                    ...
                    {
                        status = cls.ExecuteIntScalar("select count(1) from tbl_CRMLogicNote where ID=" + Convert.ToInt32(Request.QueryString["ID"]) + " and IsCommitteeApproved=1");
                        if (status == 0)
                        {
                    ...
                    {
                        int count = cls.ExecuteIntScalar("select count(1) from tbl_CRMApprover where ApproverID=" + Convert.ToInt32(dtUser.Rows[0]["ID"]) + " and CRMID=" + Convert.ToString(Request.QueryString["ID"]) + " and IsApprove=0");
                        if (count == 1)
                        {
    ...
    private void FillPaymentPlan(string ID)
    {
        ...
        DataSet ds = cls.GetDataSet("EXEC sp_ManageCRMLogicNote 'View','" + ID + "'");
    ...
    {
        DataSet dtData = cls.GetDataSet("EXEC sp_ManageCRMLogicNote 'View','" + ID + "'");
            ...
            for (int i = 0; i < dtData.Tables[6].Rows.Count; i++)
            {
                ...
                if (dtData.Tables[6].Rows[i]["Category"].ToString() == "Delivery Schedule")
                {
                    if (dtData.Tables[6].Rows[i]["DocFile"].ToString() != "")
                    {
                        ...
                        lnkdeliveryschedule.NavigateUrl = "../Upload/CRM/" + dtData.Tables[6].Rows[i]["DocFile"].ToString();
                        //lnkbidevaluation.Attributes.Add("href", dtData.Tables[4].Rows[i]["DocFile"].ToString());
                    }
                    //else
                    //{
                ...
                if (dtData.Tables[6].Rows[i]["Category"].ToString() == "Budget Approval")
                {
                    if (dtData.Tables[6].Rows[i]["DocFile"].ToString() != "")
                    {
                        ...
                        lnkbudgetapproval.NavigateUrl = "../Upload/CRM/" + dtData.Tables[6].Rows[i]["DocFile"].ToString();
                        //lnkbidevaluation.Attributes.Add("href", dtData.Tables[4].Rows[i]["DocFile"].ToString());
                    }
                    //else
                    //{
                ...
                if (dtData.Tables[6].Rows[i]["Category"].ToString() == "Prior Approvals")
                {
                    ...
                    if (dtData.Tables[6].Rows[i]["DocFile"].ToString() != "")
                    {
                        ...
                        lnkPriorApprovalsDownload.NavigateUrl = "../Upload/CRM/" + dtData.Tables[6].Rows[i]["DocFile"].ToString();
                        //lnkbidevaluation.Attributes.Add("href", dtData.Tables[4].Rows[i]["DocFile"].ToString());
                    }
                    //else
                    //{
                ...
                if (dtData.Tables[6].Rows[i]["Category"].ToString() == "Indent")
                {
                    if (dtData.Tables[6].Rows[i]["DocFile"].ToString() != "")
                    {
                        ...
                        lnkIndentdownload.NavigateUrl = "../Upload/CRM/" + dtData.Tables[6].Rows[i]["DocFile"].ToString();
                        //lnkbidevaluation.Attributes.Add("href", dtData.Tables[4].Rows[i]["DocFile"].ToString());
                    }
                    //else
                    //{
                ...
                if (dtData.Tables[6].Rows[i]["DocFile"].ToString() != "")
                {
                    ...
                    lnklogicnoteDownload.NavigateUrl = "../Upload/CRM/" + dtData.Tables[6].Rows[i]["DocFile"].ToString();
                    //lnkbidevaluation.Attributes.Add("href", dtData.Tables[4].Rows[i]["DocFile"].ToString());
                }
                //else
                //{
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CRMApproverApprove','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('CRM Logic Note Successfully Approved !!');location.replace('ListApproveCRMLogicNote.aspx');", true);
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CRMApproverReject','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('CRM Logic Note Successfully Rejected !!');location.replace('ListApproveCRMLogicNote.aspx');", true);
    ...
    protected void btnprint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Export.ExportReport(Request.QueryString["ID"].ToString(), "CRM Logic Note");
            //string ReportType = "P";//
            ...
            //_reportViewer.ServerReport.ReportPath = "/Reports/CRMlogicNote";
    ...
    public void DownloadDocImage(string ID, string CRMID, string DocFile, string Type)
    {
        if (DocFile != "" && File.Exists(Server.MapPath("../Upload/CRM/") + DocFile))
        {
            DataTable dtCRMOrderNo = new DataTable();
            dtCRMOrderNo = cls.selectDataTable(" select b.CRMOrderNo from tbl_CRMApprover a inner join tbl_CRMLogicNote b on a.CRMID=b.ID Where a.IsApprove=1 And  a.CRMID='" + CRMID + "' and a.IsDelete=0");
            if (dtCRMOrderNo.Rows.Count > 0)
            {
                string filePath = Server.MapPath("../Upload/CRM/") + ID + CRMID + Type + DocFile;
                try
                {
                    if (Type == "PDF")
                    {
                        ...
                        for (int i = 1; i <= n; i++)
                        {
                            ...
                            ColumnText.ShowTextAligned(over, Element.ALIGN_CENTER, new Phrase(dtCRMOrderNo.Rows[0]["CRMOrderNo"].ToString(), new iTextSharp.text.Font(iTextSharp.text.Font.TIMES_ROMAN, 15, iTextSharp.text.Font.BOLD)), pagesize.Width - 250, pagesize.Height - 30, 0);
                            over.RestoreState();
                        }
                        stamper.Close();
                        reader.Close();
                        ...
                        using (var bitmap = (Bitmap)System.Drawing.Image.FromFile(imageFilePath))//load the image file
                        {
                            using (Graphics graphics = Graphics.FromImage(bitmap))
                            {
                                using (System.Drawing.Font arialFont = new System.Drawing.Font("TIMES_ROMAN", 12, FontStyle.Bold))
                                {
                                    ...
                                    graphics.DrawString(dtCRMOrderNo.Rows[0]["CRMOrderNo"].ToString(), arialFont, Brushes.DarkBlue, rect1);
                                }
                            }
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CRMCommitteeApprove','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('CRM Logic Note Successfully Approved !!');location.replace('ListApproveCRMLogicNote.aspx');", true);
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CRMCommitteeReject','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('CRM Logic Note Successfully Rejected !!');location.replace('ListApproveCRMLogicNote.aspx');", true);
