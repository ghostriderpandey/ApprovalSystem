<%@ Page Title="" Language="C#" MasterPageFile="~/AdminPanel/AdminPanel.master" AutoEventWireup="true" CodeFile="ListApproveMarketingLogicNote.aspx.cs" Inherits="AdminPanel_ListApproveMarketingLogicNote" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <ContentTemplate>--%>
                ...
                <div class="w-100">
                  <asp:GridView ID="gvUser" runat="server" ShowHeaderWhenEmpty="true" EmptyDataText="No Record Found" AutoGenerateColumns="False" CssClass="table table-responsive table-responsive" OnRowCommand="gvUser_RowCommand">
                    ...
                    <asp:BoundField DataField="MLNOrderNo" HeaderText="SerialCode" />
                    ...
                    <asp:BoundField DataField="NameofProposedVendor" HeaderText="Name of Proposed Vendor" />
                    ...
                    <asp:TemplateField HeaderText="Approval Status">
                      <ItemTemplate>
                        ...
                        <a data-toggle="modal" data-target="#MyPopup" title="View this record" onClick="getApprover('<%#Eval("MLNID") %>')"><i class="fa fa-search"></i></a> </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="Action">
                      <ItemTemplate> <a href="MarketingLogicNoteReport.aspx?id=<%#Eval("MLNID") %>&AID=<%#Eval("ID") %>" title="Click to view" class="fa fa-eye  "></a>
                        <asp:LinkButton ID="lnkView" ToolTip="Click to view PDF..!" runat="server" CommandName="View" CssClass="fa fa-search" CommandArgument='<%#Eval("MLNID") %>'>
                          <%--<img src="Icons/view_16x16.png" />--%>
                        </asp:LinkButton>
                        <%--<a href="PurchaseLogicNote.aspx?id=<%#Eval("ID") %>" title="Edit this record" class="fa fa-edit">
                                                       
                                                    </a>--%>
                        <asp:LinkButton ID="lnkApprove" ToolTip="Click to approve..!" CssClass="fa fa-check-square" runat="server" Style='<%#Eval("ApprovalStatus").ToString()=="Pending"?"Display:contents": "Display:none" %>' OnClientClick="return ApproveItem();" Text="Approve" CommandName="Approve" CommandArgument='<%#Eval("ID") %>'> </asp:LinkButton>
                        <asp:LinkButton ID="lnkReject" ToolTip="Click to reject..!" CssClass="fa fa-close" runat="server" OnClientClick="return RejectItem();" Style='<%#Eval("ApprovalStatus").ToString()=="Pending"?"Display:contents": "Display:none" %>' Text="Reject" CommandName="Reject" CommandArgument='<%#Eval("ID") %>'> </asp:LinkButton>
                      </ItemTemplate>
                    </asp:TemplateField>
                    </Columns>
                  </asp:GridView>
  ...
  <script type="text/javascript">
      function getApprover(id) {
          ...
          $.ajax({
              type: "POST",
              url: "ListApproveMarketingLogicNote.aspx/getApprover",
