...
public partial class AdminPanel_PurchaseLogicNoteReportCommittee : System.Web.UI.Page
{
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PLNCommitteeApprove','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'PLNCommitteeReject','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
