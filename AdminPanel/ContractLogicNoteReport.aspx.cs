...
public partial class AdminPanel_ContractLogicNoteReport : System.Web.UI.Page
{
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CLNApproverApprove','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
    ...
    {
        int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'CLNApproverReject','" + Request.QueryString["AID"].ToString() + "','" + txtremark.Text + "'");
        if (count > 0)
        {
