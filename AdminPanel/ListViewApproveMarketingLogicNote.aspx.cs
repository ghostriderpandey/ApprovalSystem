...
public partial class AdminPanel_ListViewApproveMarketingLogicNote : System.Web.UI.Page
{
    ...
    void loadgvUser()
    {
        ...
        dtData = cls.selectDataTable("EXEC sp_ManageMarketingLogicNote 'GetViewAllforApproval','" + dtUser.Rows[0]["ID"].ToString() + "',@ProjectID='" + ddlprojectname.SelectedValue + "',@ApprovalStatus='" + ddlStatus.SelectedValue + "',@ApprovalTodate='" + txtToApprovalDate.Text + "',@ApprovalFromdate='" + txtFromApprovalDate.Text + "'");
    ...
    protected void gvUser_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'MLNApproverApprove','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Marketing Logic Note Successfully Approved !!');", true);
                loadgvUser();
        ...
        {
            int count = cls.ExecuteQuery("Exec Sp_ManageApproval 'MLNApproverReject','" + e.CommandArgument + "','" + txtRemark.Text + "'");
            if (count > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Marketing Logic Note Successfully Rejected !!');", true);
                loadgvUser();
            }
            ClientScript.RegisterStartupScript(this.GetType(), "Popup", "fnn();", true);
        }
        ...
        else if (e.CommandName == "View")
        {
            try
            {
                Export.ExportReport(e.CommandArgument.ToString(), "Marketing Logic Note");
                //string ReportType = "P";
                //Warning[] warnings;
                //string[] streamIds;
                ...
                //_reportViewer.ServerReport.ReportPath = "/Reports/Marketing Logic Note";
    ...
    public static List<Approvar> getApprover(string id)
    {
        cls_connection_new cls = new cls_connection_new();
        DataTable dtData = new DataTable();
        dtData = cls.selectDataTable("EXEC sp_ManageMarketingLogicNote 'GetAllApprover','" + id + "'");
        List<Approvar> listApprovar = new List<Approvar>();
        if (dtData.Rows.Count > 0)
        {
            for (int i = 0; i < dtData.Rows.Count; i++)
